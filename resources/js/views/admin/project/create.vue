                ...
                <template #content>
                    ...
                    <div class="col-md-3">
                        <date-picker id="date1" v-model="data.start_date" field="data.start_date" title="Start Date"
                            placeholder="Start Date" col="12" :req="true"></date-picker>
                    </div>
                    <div class="col-md-3">
                        ...
                        <template #content>
                            ...
                            <div class="col-md-6">
                                <Input v-model="indicator.title" field="indicators[indicatorIndex].title" title="Title"
                                    col="12" @input="titleRequre(indicatorIndex)" />
                            ...
                            <Switch v-model="indicator.status" field="indicators[indicatorIndex].status" title="Status"
                                on-label="Active" off-label="Deactive" col="3"></Switch>
                        </template>
                    </Fieldset>
                </div>
...
export default {
    ...
    methods: {
            ...
            this.$validate().then((res) => {
                ...
                this.indicatorErrors.map(indicatorError => {
                    ...
                    if (indicatorError.title?.length > 0) {
                        console.log(indicatorError.title?.length);
                        errorCount++;
                    }
                    if (indicatorError.total_mark?.length > 0) {
                        console.log("err", indicatorError.total_mark);
                        errorCount++;
                    }
        ...
        deleteIndicator(index) {
            ...
            this.indicatorErrors.splice(index, 1);
            this.data.indicators.forEach((indicator, i) => {
                this.titleRequre(i);
            });
        },
        ...
        titleRequre(index) {
            ...
            if (description || achivement_percentage || total_mark) {
                if (!title) {
            ...
            if (!total_mark) {
                this.indicatorErrors[index].total_mark = "Total Mark is required";
            } else {
                if (!/^\d+(\.\d+)?$/.test(total_mark) || total_mark < 1) {
                    this.indicatorErrors[index].total_mark = "Must be a positive number and greater than 0";
                } else {
                    this.indicatorErrors[index].total_mark = "";
                }
            }
            if (achivement_percentage) {
                if (!/^\d+(\.\d+)?$/.test(achivement_percentage) || achivement_percentage < 0) {
                    this.indicatorErrors[index].achivement_percentage = "Must be a positive number";
                } else if (achivement_percentage > 100) {
                    this.indicatorErrors[index].achivement_percentage = "Cannot exceed 100";
                } else if (achivement_percentage > total_mark) {
                    this.indicatorErrors[index].achivement_percentage = "Achievement Percentage cannot be greater than Total Mark";
                } else {
            ...
            const duplicateTitles = this.data.indicators.filter((indicator, i) => indicator.title === title && i !== index);
        ...
        // api section
        getStates() {
            axios.get("/get-states", { params: { status: "active" } }).then((res) => (this.allStates = res.data));
        },
        ...
        getDistricts(id) {
            axios.get("/get-districts", { params: { id: id, status: "active" } }).then((res) => (this.allDistricts = res.data)).catch(error => console.error('Error fetching districts:', error));;
        },
        ...
        getUpazillas(id) {
            axios.get("/get-upazillas", { params: { id: id, status: "active" } }).then((res) => (this.allUpazillas = res.data)).catch(error => console.error('Error fetching upazillas:', error));
        },
    },
    ...
    validators: {
        ...
        "data.start_date": function (value = null) {
            ...
            return Validator.value(value)
                .custom(function () {
                    if (that.data.project_name && Validator.isEmpty(value)) {
