    ...
    <ul class="list-unstyled sub_menu">
        <li v-for="(child_menu, index) in child_menus" :key="`sub_menu_${index}`">
            ...
            <template v-else>
                ...
                }" :class="[routeActiveClass(child_menu.route_name)]" data-bs-toggle="tooltip"
                    data-bs-placement="right" :data-bs-title="child_menu.menu_name"
...
export default {
    ...
    methods: {
        routeActiveClass(menuRouteName) {
            let classNames = "";
            if (menuRouteName?.startsWith('report.')) {
                if (this.$route.name === menuRouteName) {
                    classNames = "router-link-active active";
                }
            } else if (menuRouteName.startsWith(this.activeRouteNamePrefix)) {
                classNames = "router-link-active active";
            }
