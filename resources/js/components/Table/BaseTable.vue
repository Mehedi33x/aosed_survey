                            ...
                            <slot name="columns">
                                <th v-for="(column, index) in table.columns" :key="'a' + index"
                                    ...
                                    <span class="heading">
                                        ...
                                        <img v-if="
                                            column.enable_data_sorting == null ||
                                            column.enable_data_sorting === true
                                        " class="icon-up" :src="`${asset}icon-up.svg`" alt="icon-up" />
                                    </span>
                                </th>
                                ...
                                " :class="table.routes.array ? 'action-extra' : 'action'"></th>
                            </slot>
                        </tr>
                    </thead>
                    <tbody v-if="Object.keys(table.datas).length > 0">
                        <tr v-for="(item, index) in table.datas" :key="index" class="td_track change_sorting"
                            ...
                            <slot v-for="(column, index) in table.columns" :name="column.field" :item="item">
                                <td :key="'b' + index" v-if="hasValue(item, column.field)"
                                    @click="pushToViewRoute(item.id, column.sorting)"
                                    :style="'text-align:' + column.align">
                                    <!-- :style="'text-align:' + column.align" -->
                                    <span v-if="column.date">
                                        {{ $filter.enFormat(itemValue(item, column.field) ?? "N/A") }}
                                    </span>
                                    ...
                                    <template v-else-if="column.image">
                                        ...
                                        <img v-else :src="itemValue(item, column.field) ?? emptyImage50()"
                                            :href="itemValue(item, column.zoom_field ?? column.field)" v-x-zoom-image="itemValue(item, column.field) &&
                                                itemValue(item, column.zoom_field ?? column.field)
                                                ? true
                                                : false
                                                " :style="'width:' + column.imgWidth + ';height:' + column.height" />
                                    ...
                                    <template v-else-if="column.html">
                                        <span v-html="itemValue(item, column.field)"></span>
                                    </template>
                                    <span v-else-if="column.field == 'status'">
                                        <span class="status approved" v-if="
                                            itemValue(item, column.field, column.subfield) == 'active' ||
                                            itemValue(item, column.field, column.subfield) == 1
                                        ">ACTIVE</span>
                                        <span class="status pending" v-if="
                                            itemValue(item, column.field, column.subfield) == 'draft' ||
                                            itemValue(item, column.field, column.subfield) == 0
                                        ">DRAFT</span>
                                        <span class="status cancel" v-if="
                                            itemValue(item, column.field, column.subfield) == 'deactive' ||
                                            itemValue(item, column.field, column.subfield) == 0
                                        ">DEACTIVE</span>
                                        <span class="status pending"
                                            v-if="itemValue(item, column.field, column.subfield) == 'ur'">UNREAD</span>
                                        <span class="status approved"
                                            v-if="itemValue(item, column.field, column.subfield) == 'r'">READ</span>
                                        <span class="status cancel"
                                            v-if="itemValue(item, column.field, column.subfield) == 'restored'">RESTORED</span>
                                        <span class="status pending"
                                            v-if="itemValue(item, column.field, column.subfield) == 'pending'">PENDING</span>
                                        <span class="status approved"
                                            v-if="itemValue(item, column.field, column.subfield) == 'success'">SUCCESS</span>
                                    </span>
                                    <template v-else>
                                        <span v-if="column.to">
                                            ...
                                            }" data-bs-toggle="tooltip" data-bs-placement="top" :data-bs-title="`${itemValue(item, column.field, column.subfield) ?? ''
                                                }`" v-x-tooltip="safeStringLength(
                                                    itemValue(item, column.field, column.subfield)
                                                ) >= tooltipLength
                                                    ">{{
                                        ...
                                        <span v-else data-bs-toggle="tooltip" data-bs-placement="top" :data-bs-title="`${itemValue(item, column.field, column.subfield) ?? ''
                                            }`" v-x-tooltip="safeStringLength(
                                                itemValue(item, column.field, column.subfield)
                                            ) >= tooltipLength
                                                ">
...
export default {
    ...
    methods: {
        ...
        tableSortingByButton(id, sorting = null, model) {
            ...
            setTimeout(() => $(".change_sorting").removeClass("sorting-success"), 5000);
        },
        ...
        generatePdf() {
            ...
            doc.setFontSize(16);
            const title = `${this.ucfirst(process.env.MIX_VUE_APP_NAME)} ${this.ucfirst(
                this.model
            )}`;
        ...
        goPrevAndNext(pageValue = 1) {
            ...
            if (goToPage > 0 && goToPage != currentPage && goToPage <= lastPage) {
                ...
                axios
                    ...
                    .catch((error) => setTimeout(() => (this.$root.tableSpinner = false), 200));
            }
        },
        ...
        prevAndNextPageButtonClass() {
            if (this.table.meta.last_page === 1) {
                ...
                this.back_prev_page_class = "";
            } else if (this.table.meta.current_page === this.table.meta.last_page) {
                this.go_next_page_class = "";
